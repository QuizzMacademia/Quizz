...
public class JSONLoader<T> {
	public static final String QUIZZ_SERVER_PATH = "/quizz.server/";
	...
	public JSONLoader(String path, Class<T[]> arrayType, Class<T> classType, JpaRepository<T, Long> repository,
	...
	private Gson getGson() {
		if (this.adapter != null) {
			return new GsonBuilder().setPrettyPrinting().registerTypeAdapter(this.classType, this.adapter).create();
		} else {
	...
	public List<T> load() {
		...
		final String cwdDirectory = resourceDir.substring(0, resourceDir.lastIndexOf(QUIZZ_SERVER_PATH));
		final String url = cwdDirectory + QUIZZ_SERVER_PATH + this.path;
